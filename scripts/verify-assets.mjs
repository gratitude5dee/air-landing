...
async function verifyClosingArtwork() {
  const absolutePath = resolve(PUBLIC_ROOT, `.${CLOSING_ART_PATH}`);
  if (!isContainedPath(PUBLIC_ROOT, absolutePath)) {
    fail(`Closing artwork escapes the public directory: ${CLOSING_ART_PATH}`);
  }
  const fileStats = await stat(absolutePath).catch(() => null);
  if (!fileStats?.isFile()) fail(`Closing artwork is missing: ${CLOSING_ART_PATH}`);
  if (fileStats.size > CLOSING_ART_BUDGET) {
    fail(`Closing artwork exceeds ${CLOSING_ART_BUDGET} bytes (${fileStats.size}).`);
  }
  ...
  try {
    ...
    fail(`Closing artwork is not a readable AVIF: ${error instanceof Error ? error.message : String(error)}`);
  }
  if (metadata.mediaType !== "image/avif" || metadata.format !== "heif") {
    fail(`Closing artwork must be AVIF: ${CLOSING_ART_PATH}`);
  }
  if (metadata.width !== 1672 || metadata.height !== 941) {
    fail(`Closing artwork must be 1672x941, received ${String(metadata.width)}x${String(metadata.height)}.`);
  }
  if (metadata.pages && metadata.pages !== 1) fail("Closing artwork must be a single frame.");
