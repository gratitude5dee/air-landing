...
The WZRD landing popup submits to the same waitlist API with an allowed `Origin` of `https://wzrd.tech` (configure `AIR_WAITLIST_ALLOWED_ORIGINS` for additional preview origins). The public endpoints return only receipt and aggregate position data; contact details remain behind `/admin`. The admin source filter treats legacy `air-landing` entries as Air entries so older records remain visible.
